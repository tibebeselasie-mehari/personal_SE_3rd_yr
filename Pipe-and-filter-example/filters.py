...
from print_funcs import print_header, print_indented, print_line, print_underlined
...
def show_report(inputs):
    ...
    totals_counter = reduce(lambda x, y: len(
        list(x)[1]) + len(list(y)[1]), inputs)

    print_underlined("Overall summary:", indentation=1)

    print_indented("<{:.>4}> file contents".format(len(inputs)), indentation=2)
    print_indented("<{:.>4}> files are found to be duplicate.".format(
        totals_counter), indentation=2, end="\n\n")

    print_underlined("Detailed Results:", end="\n", indentation=1)

    for content, files in inputs:
        print_indented("[+] Content : '", content[:50],
                       end=" ... '\n", indentation=2)
        print_indented("[+] <%d> files share the above content: " %
                       (len(list(files))), indentation=2)
